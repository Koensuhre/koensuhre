  ...
  useEffect(() => {
    ...
    const move = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        document
          .querySelectorAll<HTMLElement>("[data-scroll-speed]")
          .forEach((element) => {
            if (!wide) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <nav
          className="mx-auto flex h-20 max-w-[1380px] items-center justify-between px-6 lg:px-10"
        ...
        {menuOpen && (
          <div className="absolute inset-x-0 top-20 border-t border-border bg-background p-6 shadow-soft md:hidden">
            {nav.map(([label, id]) => (
              <button
                key={id}
                onClick={() => go(id)}
                className="block w-full border-b border-border py-4 text-left font-display text-2xl"
              >
                {label}
              </button>
            ))}
      ...
      <main>
        <section
          id="home"
          className="relative flex min-h-[92vh] items-center overflow-hidden border-b-2 border-foreground px-6 pb-14 pt-28 lg:px-10"
        >
          <div
            className="kinetic-disc absolute -right-12 top-28 size-56 sm:-right-24 sm:size-72 md:size-[30rem]"
          ...
          <div className="mx-auto w-full max-w-[1380px]">
            <div className="mb-8 flex items-center justify-between border-b-2 border-foreground pb-4 text-xs font-medium uppercase">
              ...
              <span className="hidden sm:block">Almelo — Nederland</span>
            </div>

            <h1 className="hero-type relative z-10 font-bold uppercase">
              <span className="hero-line hero-line-left">Web</span>
              <span className="hero-line hero-line-right text-primary">
                Designer
              </span>
            ...
            <div className="mt-10 grid gap-8 border-t-2 border-foreground pt-7 md:grid-cols-3 md:items-end">
              <p className="max-w-sm text-lg leading-snug">
                Ik ontwerp en bouw digitale ervaringen waarin beeld, techniek
                en verhaal samenkomen.
              </p>

              <p className="text-sm leading-6 text-muted-foreground md:col-start-2">
                Webdesign · development
                <br />
                fotografie · film · strategie
                <br />
                SEO · content · online marketing
              </p>

              <button
                onClick={() => go("cases")}
                className="group ml-auto flex size-24 items-center justify-center rounded-full border-2 border-foreground transition-colors hover:bg-foreground hover:text-background"
                aria-label="Bekijk mijn werk"
              >
                <ArrowDown className="size-7 transition-transform group-hover:translate-y-2" />
              </button>
            </div>
          </div>
        </section>
        ...
        <div className="marquee" aria-hidden="true">
          <div className="marquee-track">
            DESIGN — CODE — BEELD — IDEEËN — DESIGN — CODE — BEELD — IDEEËN —
          </div>
        </div>

        <section
          id="over"
          className="overflow-hidden border-b-2 border-foreground px-6 py-28 lg:px-10 lg:py-44"
        >
          <div className="mx-auto grid max-w-[1380px] gap-14 lg:grid-cols-12">
            ...
            <div className="reveal lg:col-span-8">
              <h2 className="section-title max-w-6xl uppercase">
                Tussen idee en{" "}
                <span className="serif-cut text-primary">uitvoering</span>{" "}
                voel ik me thuis.
              </h2>

              <p className="mt-10 max-w-2xl text-lg leading-8 text-muted-foreground">
                Ik ben webdesigner, developer en beeldmaker. Daardoor kan ik
                een project als geheel bekijken: wat het moet vertellen, hoe
                het moet voelen en hoe het technisch prettig blijft werken.
                Soms begint dat met een gesprek, soms met een camera of een
                schets.
              </p>
            </div>
          </div>
        ...
        <section
          id="diensten"
          className="border-b-2 border-foreground bg-signal px-6 py-28 text-signal-foreground lg:px-10 lg:py-40"
        >
          <div className="mx-auto max-w-[1380px]">
            <div className="reveal max-w-6xl">
              <Eyebrow text="Wat ik maak" />

              <h2 className="section-title mt-7 uppercase">
                Verschillende{" "}
                <span className="serif-cut">disciplines.</span>
                <br />
                Eén handschrift.
              </h2>
            </div>
            ...
            <div className="mt-20 grid gap-x-12 md:grid-cols-2">
              {disciplines.map(([number, title, text], index) => (
                <article
                  key={title}
                  className={`reveal border-t-2 border-signal-foreground py-10 ${
                    index % 2 === 1 ? "md:translate-y-16" : ""
                  }`}
                >
                  <span className="text-xs">{number}</span>

                  <h3 className="mt-6 text-2xl font-bold uppercase md:text-3xl">
                    {title}
                  </h3>
        ...
        <section id="cases" className="px-6 py-28 lg:px-10 lg:py-44">
          <div className="mx-auto max-w-[1380px]">
            <div className="reveal flex flex-col justify-between gap-10 md:flex-row md:items-end">
              <div>
                <Eyebrow text="Geselecteerd werk" />

                <h2 className="section-title mt-7 uppercase">
                  Werk/<span className="serif-cut text-primary">04</span>
                </h2>
              </div>

              <div className="max-w-md">
                <p className="leading-7 text-muted-foreground">
                  Een selectie waarin strategie, ontwerp, techniek en beeld
                  elkaar aanvullen.
                </p>

                <div
                  className="mt-7 inline-flex border-2 border-foreground p-1"
                  role="tablist"
                  aria-label="Soort projecten"
                >
                  <Button
                    role="tab"
                    aria-selected={projectType === "web"}
                    variant={projectType === "web" ? "default" : "ghost"}
                  ...
                  <Button
                    role="tab"
                    aria-selected={projectType === "media"}
                    variant={projectType === "media" ? "default" : "ghost"}
                    className="rounded-none"
                    onClick={() => setProjectType("media")}
                  >
                    Foto & video
                  </Button>
                </div>
            ...
            {projectType === "web" ? (
              <div
                className="mt-24 space-y-36"
                role="tabpanel"
                aria-label="Websiteprojecten"
              >
                {cases.map((project, index) => (
                  ...
                  >
                    <div
                      className={`project-frame group relative lg:col-span-8 ${
                        index % 2 === 1
                          ? "lg:order-2 lg:translate-x-10"
                          : "lg:-translate-x-10"
                      }`}
                      data-scroll-speed={
                        index % 2 === 1 ? "-0.035" : "0.035"
                      }
                    >
                      <img
                        src={project.image}
                        ...
                        decoding="async"
                        className="block aspect-[16/10] h-auto w-full max-w-full object-cover object-top transition duration-1000 group-hover:scale-[1.04]"
                      />

                      <span className="absolute left-4 top-4 bg-signal px-4 py-3 font-display text-xl font-bold text-signal-foreground">
                        {project.number}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      <h3 className="mt-5 text-4xl font-bold uppercase md:text-6xl">
                        {project.name}
                      </h3>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="story-link mt-8 inline-flex items-center gap-2 pb-1 text-sm"
                      >
                        {project.url}
              ...
              <div
                ...
                role="tabpanel"
                aria-label="Foto- en videoprojecten"
              >
                <div className="grid gap-10 md:grid-cols-12 md:items-end">
                  <p className="text-xs font-semibold uppercase text-primary md:col-span-3">
                    Film & beeld
                  </p>

                  <h3 className="text-4xl font-bold uppercase leading-none md:col-span-9 md:text-7xl">
                    Bewegend beeld, eerlijk verteld.
                  </h3>
                </div>
                ...
                <div className="grid gap-x-8 gap-y-16 md:grid-cols-2">
                  {videos.map((video) => (
                    <article key={video.title}>
                      <div className="project-frame aspect-video w-full overflow-hidden">
                        <iframe
                          src={video.embed}
                      ...
                      <p className="mt-5 text-xs uppercase text-primary">
                        {video.note}
                      </p>

                      <h4 className="mt-2 font-display text-2xl font-bold uppercase md:text-3xl">
                        {video.title}
                      </h4>
                    </article>
                  ))}
        ...
        <section
          id="aanpak"
          className="bg-foreground px-6 py-28 text-background lg:px-10 lg:py-40"
        >
          <div className="mx-auto max-w-[1380px]">
            <h2 className="section-title max-w-6xl uppercase">
              Rust in het proces.
              <br />
              <span className="serif-cut text-signal">
                Ruimte voor ideeën.
              </span>
            </h2>
          </div>
        ...
        <section id="contact" className="px-6 pb-20 lg:px-10 lg:pb-32">
          <div className="mx-auto overflow-hidden rounded-[2.5rem] bg-secondary p-7 md:p-14 lg:p-20">
            <div className="grid gap-16 lg:grid-cols-2">
              <div className="reveal">
                <Eyebrow text="Contact" />

                <h2 className="mt-7 text-5xl font-medium leading-tight md:text-7xl">
                  Heb je iets
                  <br />
                  in gedachten?
                </h2>
                ...
                <p className="mt-7 max-w-md text-lg leading-8 text-muted-foreground">
                  Vertel me waar je aan werkt. Dan kijken we rustig of mijn
                  manier van werken erbij past.
                </p>

                <a
                  href="mailto:koensuhre@gmail.com"
                  className="mt-10 inline-block border-b border-foreground pb-1 text-lg"
                >
                  koensuhre@gmail.com
                </a>

                <a
                  href="tel:+31623816297"
                  className="mt-3 block text-lg text-muted-foreground transition-colors hover:text-foreground"
                >
                  06 238 16 297
                </a>
              </div>
              ...
              >
                ...
                <label className="block text-sm">
                  ...
                  <textarea
                    ...
                    name="message"
                    rows={5}
                    placeholder="Een korte omschrijving is genoeg"
                    className="mt-2 w-full resize-none rounded-2xl border border-border bg-card p-4 outline-none placeholder:text-muted-foreground/60 focus:border-primary"
                  />
                </label>
                ...
                {sent && (
                  ...
                  >
                    ...
                    Je bericht staat klaar. De verzendservice moet nog worden
                    gekoppeld.
                  </p>
                )}
              </form>
      ...
      <footer className="px-6 pb-10 lg:px-10">
        <div className="mx-auto flex max-w-[1380px] flex-col gap-8 border-t border-border pt-9 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="font-display text-xl font-medium">Koen Suhre</p>

            <p className="mt-2 text-sm text-muted-foreground">
              Design · Development · Fotografie · Film
            </p>
...
function Eyebrow({
  ...
  return (
    <p
      className={`flex items-center gap-3 text-xs font-medium uppercase ${
        light ? "text-background/60" : "text-muted-foreground"
      }`}
    >
      <span
        className={`h-px w-9 ${light ? "bg-signal" : "bg-primary"}`}
      />
      {text}
    </p>
  );
  ...
  return (
    <label className="block text-sm">
      ...
      <input
        ...
        className="mt-2 h-12 w-full rounded-full border border-border bg-card px-5 outline-none placeholder:text-muted-foreground/60 focus:border-primary"
      />
    </label>
  );
}
...
function SkillSlider() {
  ...
  const move = (direction: -1 | 1) => {
    ...
    const next = Math.max(
      0,
      Math.min(skillGroups.length - 1, active + direction),
    );
    ...
    if (width > 0) {
      setActive(
        Math.min(
          ...
          Math.round(rail.scrollLeft / width),
        ),
      );
    }
  };
  ...
  return (
    <section
      id="skills"
      className="overflow-hidden border-b-2 border-foreground bg-secondary py-28 lg:py-40"
      aria-labelledby="skills-title"
    >
      <div className="mx-auto max-w-[1380px] px-6 lg:px-10">
        <div className="reveal flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
          <div>
            <Eyebrow text="Tools & platforms" />

            <h2
              id="skills-title"
              className="section-title mt-7 max-w-5xl uppercase"
            >
              Gereedschap voor{" "}
              <span className="serif-cut text-primary">ideeën.</span>
            </h2>
          </div>
          ...
          <div className="flex items-center gap-3 self-end">
            ...
            <span
              ...
              aria-live="polite"
            >
              0{active + 1} / 0{skillGroups.length}
            </span>
            ...
            <Button
              ...
              size="icon"
              onClick={() => move(1)}
              disabled={active === skillGroups.length - 1}
              aria-label="Volgende skillgroep"
            >
              <ArrowRight />
            </Button>
          </div>
      ...
      <div
        ...
        tabIndex={0}
        role="region"
        aria-label="Skillgroepen"
        className="skill-rail mt-16 flex gap-5 overflow-x-auto px-6 pb-8 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary lg:gap-8 lg:px-[max(2.5rem,calc((100vw-1380px)/2))]"
      >
        {skillGroups.map((group) => (
          <article
            key={group.label}
            className="skill-slide relative flex min-h-[30rem] w-[84vw] shrink-0 flex-col justify-between overflow-hidden border-2 border-foreground bg-background p-7 md:w-[58vw] md:p-10 lg:w-[48rem]"
          >
            <div className="flex items-start justify-between gap-6">
              ...
              <span className="skill-number text-7xl leading-none text-primary/25 md:text-9xl">
                {group.number}
              </span>
            </div>

            <div>
              <h3 className="max-w-xl text-3xl font-bold uppercase leading-tight md:text-5xl">
                {group.lead}
              </h3>

              <ul
                className="mt-10 flex flex-wrap gap-2"
                aria-label={`Programma's voor ${group.label}`}
              >
      ...
      <div className="mx-auto mt-3 flex max-w-[1380px] items-center gap-5 px-6 lg:px-10">
        ...
        <div className="h-1 flex-1 overflow-hidden bg-border">
          <div
            ...
            style={{
              width: `${((active + 1) / skillGroups.length) * 100}%`,
            }}
          />
        </div>
      </div>
