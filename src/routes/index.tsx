      ...
      <main>
        ...
        <section id="cases" className="px-6 py-28 lg:px-10 lg:py-44">
          <div className="mx-auto max-w-[1380px]">
            ...
            {projectType === "web" ? (
              <div className="mt-24 space-y-36" role="tabpanel" aria-label="Websiteprojecten">{cases.map((project, index) => <article key={project.name} className="grid items-center gap-10 lg:grid-cols-12"><div className={`project-frame group lg:col-span-8 ${index % 2 ? "lg:order-2 lg:translate-x-10" : "lg:-translate-x-10"}`} data-scroll-speed={index % 2 ? "-0.035" : "0.035"}><img src={project.image} alt={`Website van ${project.name}`} width={1280} height={800} loading="lazy" className="aspect-[16/10] w-full object-cover object-top transition duration-1000 group-hover:scale-[1.04]" /><span className="absolute left-4 top-4 bg-signal px-4 py-3 font-display text-xl font-bold text-signal-foreground">{project.number}</span></div><div className={`relative z-10 lg:col-span-4 ${index % 2 ? "lg:order-1" : "lg:col-start-9"}`}><p className="text-xs uppercase text-primary">{project.note}</p><h3 className="mt-5 text-4xl font-bold uppercase md:text-6xl">{project.name}</h3><p className="mt-6 text-base leading-8 text-muted-foreground md:text-lg">{project.text}</p><a href={`https://${project.url}`} target="_blank" rel="noreferrer" className="story-link mt-8 inline-flex items-center gap-2 pb-1 text-sm">{project.url} <ExternalLink className="size-4" /></a></div></article>)}</div>
            ) : (
